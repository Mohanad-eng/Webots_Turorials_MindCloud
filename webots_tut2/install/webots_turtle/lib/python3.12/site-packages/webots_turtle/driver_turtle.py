...
class RobotDriver:
    def init(self, webots_node, properties):
        ...
        self._motors = {}
        self._motor_names = [ 'left wheel motor', 'right wheel motor']
    ...
    def step(self):
        ...
        self._motors['right wheel motor'].setVelocity(v_right)
        self._motors['left wheel motor'].setVelocity(v_left)

        msg = JointState()
        msg.header = Header()
        msg.header.stamp = self._node.get_clock().now().to_msg()
        msg.name = self._motor_names
        msg.velocity = [v_left, v_right]
